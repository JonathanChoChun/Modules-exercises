...
<div id="masonry-page">
	...
	<div id="grid" class="main-content">
		...
		<c:forEach var="product" items="${requestScope.products}">
			...
			<c:set var="specialTag" value=""/>
			<c:if test="${product.topSeller}">
				<c:set var="specialTag" value="top-seller"/>
			</c:if>
			<c:if test="${product.remainingStock==0}">
				<c:set var="specialTag" value="sold-out"/>
			</c:if>

		<div class="tile ${specialTag}">
			<!-- Link to the Detail page using the product id (e.g. products/detail?id=1) -->
			<c:url var="detailPath" value="/products/detail?id=${product.id}" />
			<a class="product-image" href="${detailPath}">
