...
public class Exercises {
	...
	public boolean posNeg(int a, int b, boolean negative) {
		...
		// one of them and negative is false
		if ((aIsPositive^bIsPositive)&&!negative){
			return true;
		}
		// both of them negative, negative is true
		else if(negative && !aIsPositive && !bIsPositive) {
			return true;
		}
		// both of them positive, but negative is false
		else {
			return false;
		}
	...
	public boolean cigarParty(int cigars, boolean isWeekend) {
		...
		// if NOT weekend, cigar between 40 and 60
	...
	public int dateFashion(int you, int date) {
		...
		// the things is if with either of them <=2 then return 0 (and it doesn't depend on other having the stylish >=8). (5,2) return 0.
		if (((date<=2)||(you<=2))){
			return 0;
		}
		if (((date>=8)||(you>=8))&&((date>2)&&(you>2))){
			return 2;
		}
		if (((date>=8)||(you>=8))&&((date<=2)||(you<=2))){
			return 0;
	...
	public boolean squirrelPlay(int temp, boolean isSummer) {
		...
		// if it is summer and between 60 to 100
		if (isSummer && ((temp>=60)&&(temp<=100))){
			return true;
		}
		// if not summer and between 60 to 90
