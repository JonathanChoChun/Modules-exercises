...
public class MusicCli {
    ...
    public MusicCli(MusicTransport musicTransport, Menu menu, PrintStream out, InputStream in, Logger logger) {
    ...
    private void DisplayAlbums(String genreName){
        for (Album album : musicTransport.getAlbums()) {
            if (genreName==null || genreName.equalsIgnoreCase(album.getGenre())) {
    ...
    private void DisplayAlbumDetail(Album album){
        ...
        out.println("Artist - " + (album).getArtist());
        out.println("Genre - " + (album).getGenre());
